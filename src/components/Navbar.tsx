...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 transition-colors duration-200 pt-[env(safe-area-inset-top)]">
      <div className="max-w-6xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16 gap-2 min-w-0">
          <Link
            to="/"
            className="font-display font-bold text-sm sm:text-base lg:text-xl text-sky-600 dark:text-sky-400 transition-opacity hover:opacity-90 duration-200 leading-tight min-w-0 max-w-[65%] sm:max-w-none"
            title="Health Claim Predictor"
          >
          ...
          <div className="flex lg:hidden items-center gap-1.5 shrink-0">
            <Link
              to="/signup"
              className="btn-primary px-3 py-2.5 min-h-[44px] inline-flex items-center justify-center rounded-lg text-xs sm:text-sm transition-transform duration-200 active:scale-[0.98] leading-tight whitespace-nowrap"
            >
              Get Started
            </Link>
            <button
              type="button"
              onClick={() => setOpen((o) => !o)}
              className="min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors duration-200"
              aria-label="Menu"
            >
              {open ? (
                <span className="text-xl inline-flex items-center justify-center w-10 h-10">✕</span>
              ) : (
                <span className="text-xl inline-flex items-center justify-center w-10 h-10">☰</span>
              )}
            </button>
        ...
        {open && (
          <div className="lg:hidden py-4 border-t border-slate-200 dark:border-slate-700 animate-slide-down origin-top">
